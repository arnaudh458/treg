...
async def _admin_purge_evidence(args) -> int:
    """Blank expired error evidence past the 14-day retention window.

    Replaces the side-effecting call that used to run on GET /admin/errors — a GET should never
    modify data. Schedule this as a daily cron job.
    """
    from .infra.db import verify_db
    from .routers.admin import purge_expired_error_evidence

    await verify_db()
    result = await purge_expired_error_evidence(batch_size=args.batch_size)
    print(json.dumps(result, sort_keys=True))
...
def main(argv: list[str] | None = None) -> int:
    ...
    purge.add_argument("--batch-size", type=int, default=5000,
