...
~80 curated providers (`CATALOG_VERSION`, now **20** — ScrapeGraphAI (`SGAI-APIKEY`) joined on 2026-09-24; LimaData, MoltSets, Financial Datasets, Exa, Crustdata and Aviato are included, and
...
grant (see [local-run](../architecture/local-run.md)). `CATALOG_VERSION` is now **20**. An `unsupported:true` block is first-class: it tells the analyzer
